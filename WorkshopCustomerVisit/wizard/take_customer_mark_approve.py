...
class TakeCustomerMarkApprove(models.TransientModel):
    _name = 'tr.take.customer.mark.approve.wiz'
    ...
    # Mark Approve Take Customer
    def action_mark_approve(self):
        take_customer_env = self.env['tr.take.customer']
        take_customer_mark = take_customer_env.search(
            [
                ('state', '=', 'wait')
            ]
        )

        for take_customer in take_customer_mark:
            take_customer.write(
                {
                    'state': 'approve'
                }
